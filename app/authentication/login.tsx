...
import { loginUser } from "@/app/backend-api/api"; // ✅ import API function
        ...
        Alert.alert("✅ Success", "Login successful!", [
          {
            text: "OK",
            onPress: async () => {
              // ✅ If admin → start location tracking
              if (res.user.role === "admin") {
                await AdminLocationTracker.startTracking(); // <-- Start tracking silently
    ...
    <SafeAreaProvider style={styles.container}>
      <SafeAreaView style={styles.flex}>
        <View style={[styles.inputcard, styles.paddinghorizontal, styles.gap]}>
          ...
          <TouchableOpacity onPress={() => router.push("/authentication/forgotpassword")} style={[styles.rightalign]}>
            <Text style={[styles.locationtxt,styles.fontweight]}>Forgot Password?</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
      ...
      <View style={[ styles.paddinghorizontal, styles.gap, styles.mtop, {marginBottom: 100}]}>
        ...
        <TouchableOpacity onPress={() => router.push("/authentication/register")} style={[styles.centeralign]}>
          <Text style={[styles.locationtxt,styles.fontweight]}>Don't have an account? Register</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaProvider>
  );
