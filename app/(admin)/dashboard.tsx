import {
  adminGetClients,
  adminGetVisits,
  approveOrDeclineClient, clearAllNotifications, deleteNotification,
...
function Dashboard() {
  ...
  const [selectedClient, setSelectedClient] = useState<NotificationItem | null>(null);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (Array.isArray(data.activities)) {
        ...
        const todayGroup = data.activities.find(
          (g: any) => g._id === formattedToday
        );

        const allActivities: EmployeeActivity[] = [];

        if (todayGroup && Array.isArray(todayGroup.activities)) {
          for (const act of todayGroup.activities) {
            allActivities.push({
  ...
  return (
    <SafeAreaProvider style={styles.dashboardcontainer}>
      ...
      <View style={[styles.dashboardframe, styles.gap]}>
        ...
        <Text style={[ styles.status, styles.paddinghorizontal]}>Today’s Activity</Text>

        <ScrollView>
          <View style={[styles.gap, styles.marginBottom1]}>
            {activities.map((emp, index) => (
              <TouchableOpacity key={index} style={[styles.avatarframe]} onPress={() => openModal(emp)}>
                <View style={styles.rowitem}>
                  <Image source={ emp.profilePic
                        ? { uri: emp.profilePic }
                  ...
                  <View style={[styles.centeralign, styles.gap5]}>
                    <View>
                      <Text style={styles.status}>{emp.name.replace(/\b\w/g, (c) => c.toUpperCase())}</Text>
                      <Text style={styles.color}>{emp.type.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</Text>
                    </View>
                  </View>
                </View>
              </TouchableOpacity>
            ...
            {activities.length === 0 && !loading && (
              <View style={[styles.centeralign, styles.margintop]}>
                <Text style={[styles.locationtxt,styles.color]}>No activity found today</Text>
              </View>
            )}
          </View>
        </ScrollView>
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={closeModal}>
        <View style={styles.clientbox}>
          <View style={[styles.clientframe, styles.gap]}>
            <View style={styles.rowitem}>
              <Image source={ selectedEmp?.profilePic
              ...
              <View style={[styles.gap, styles.width60]}>
                <Text style={[styles.status]}>
                 {selectedEmp?.name?.replace(/\b\w/g, (c) => c.toUpperCase())}
                </Text>
            ...
            {(selectedEmp?.type === "client-check-in" || selectedEmp?.type === "client-check-out") &&
              selectedEmp?.clientName && (<Text style={[styles.locationtxt]}>Client Name : {selectedEmp.clientName}</Text>
            )}
            {selectedEmp?.checkInTime && (
              <Text style={[styles.plantxt, styles.color]}>Check In : {selectedEmp.checkInTime}</Text>
            )}
            {selectedEmp?.checkOutTime && (
              <Text style={[styles.plantxt, styles.color]}>Check Out : {selectedEmp.checkOutTime}</Text>
            )}
            {selectedEmp?.clockInTime && (
              <Text style={[styles.plantxt, styles.color]}>Clock In : {selectedEmp.clockInTime}</Text>
            )}
            {selectedEmp?.clockOutTime && (
              <Text style={[styles.plantxt, styles.color]}>Clock Out : {selectedEmp.clockOutTime}</Text>
            )}
            {selectedEmp?.date && (
              <Text style={[styles.plantxt, styles.color]}>Date : {selectedEmp.date}</Text>
            )}
      ...
      {/* 🔔 Notifications Modal */}
      <Modal visible={notificationModalVisible} animationType="slide" onRequestClose={() => setNotificationModalVisible(false)}>
        ...
        <GestureHandlerRootView style={styles.flex}>
          <View style={[styles.container, styles.paddinghorizontal, styles.flex]}>
            ...
            {notifications.length > 0 ? (
              <>
                <TouchableOpacity style={[styles.rightalign, styles.paddinghorizontal, styles.marginBottom1]} onPress={handleClearAll}>
                  <Text style={[styles.locationtxt, styles.colorred]}>Clear All</Text>
                </TouchableOpacity>
               ...
               <FlatList
                  ...
                  renderItem={({ item: note }) => {
                    const renderRightActions = () => (
                      <View>
                        <View style={[ styles.centeralign, styles.paddinghorizontal, styles.flex, styles.paddingbottom]}>
                    ...
                    const NotificationCard = (
                      <View style={[styles.avatarframe, styles.rowitem, styles.marginBottom1]}>
                        <View style={styles.centeralign}>
                          <Image source={ note.profilePic
                                ? { uri: note.profilePic }
                                : require("@/assets/images/profile.webp")
                            } style={[styles.avatarheight, styles.empavatar]}
                          />
                        ...
                        <View>
                          {note.employeeName && (
                            <Text style={[styles.status]}>
                              {note.employeeName.replace(/\b\w/g, (c) => c.toUpperCase())}
                            </Text>
                          )}

                          <Text style={[styles.locationtxt, styles.color]}>
                            {note.type === "client-pending"? "Client Request"
                              : note.type?.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) || note.message}
                          </Text>
                        ...
                        {note.type === "client-pending" && (
                          <View style={[styles.centeralign,styles.rightalign,{ width: 63 }]}>
                            <Btn title="View" onPress={() => openClientModal(note)} />
                          </View>
                        )}
                      </View>
                    );
                    ...
                    return (
                      <Swipeable
                        ...
                        onSwipeableOpen={() => handleDeleteNotification(note._id)}
      ...
      <Modal visible={clientModalVisible} transparent animationType="slide" onRequestClose={() => setClientModalVisible(false)}>
        <View style={styles.clientbox}>
          <View style={[styles.clientframe, styles.gap]}>
            <Image source={ selectedClient?.clientData?.image?.data
            ...
            <Text style={styles.status}>Client Name : {selectedClient?.clientData?.name || "Unknown Client"}</Text>
            <Text style={[styles.locationtxt, styles.color]}>Address : {selectedClient?.clientData?.address || "N/A"}</Text>
            <Text style={[styles.locationtxt, styles.color]}>Number : {selectedClient?.clientData?.clientNumber || "N/A"}</Text>

            <View style={styles.rowitem}>
              <Btn title="Approve" onPress={() => handleApproveDecline("approve")} style={styles.width48}/>
              <Btn title="Decline" onPress={() => handleApproveDecline("decline")} style={styles.width48}/>
            </View>
          </View>
        </View>
      </Modal>
