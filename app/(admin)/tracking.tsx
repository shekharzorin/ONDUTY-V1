// Track.tsx
import {
  adminAddEmployee, adminDeleteEmployee, adminGetEmployees, adminGetVisits, API_BASE_URL, fetchEmployeeProfilePhoto, fetchVisitImageBase64,
  getAdminDashboard, getAllLiveLocations, getEmployeeLiveLocation, getEmployeeLocationHistory,
...
const fetchEmployeeProfileMeta = async (email: string): Promise<any | null> => {
  try {
    const token = await AsyncStorage.getItem("authToken");
    if (!token) throw new Error("No auth token found");
    const res = await axios.get(`${API_BASE_URL}/profile/photo/${encodeURIComponent(email)}?meta=true`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return res.data?.profile || null;
  } catch (err: any) {
    console.error(`❌ Error fetching profile meta for ${email}:`, err.response?.data || err.message);
    return null;
  }
};
...
function Track() {
  ...
  const fetchEmployeeVisits = async (
    empEmail: string,
    setEmployeeVisits: (visits: any[]) => void,
    setTotalVisitCount: (count: number) => void,
    setVisitImages: (images: Record<string, string>) => void,  // ✅ comma added
    setCompletedVisitCount: (count: number) => void
  ) => {
    try {
      const data = await adminGetVisits(empEmail);

      setEmployeeVisits(data.visits || []);
      setTotalVisitCount(data.totalVisits || 0);
      setCompletedVisitCount(data.completed || 0);  // set completed tasks
      ...
      for (const visit of data.visits || []) {
        if (visit._id) {
          try {
            ...
            if (imgBase64) newImages[visit._id] = imgBase64;
          } catch (err) {
...
// ----------------------------------- SOCKET SETUP ---------------------------------------------------------------------------------------------->
  ...
  // ✅ Single, clean socket connection
  useEffect(() => {
    console.log("📡 Socket listener attached (React version)");
    const handleLocationUpdate = (data: any) => {
      console.log("📍 Live update:", data);
      // Update reducer → triggers rerender automatically
      dispatchLocations({
        type: "update",
        payload: {
  ...
  useEffect(() => {
    const handleLocationUpdate = (data: any) => {
      dispatchLocations({ type: "update", payload: data });
    };
    socket.on("locationUpdate", handleLocationUpdate);
              ...
              <Modal visible={dropdownVisible} transparent onRequestClose={() => setDropdownVisible(false)}>
                <TouchableOpacity onPressOut={() => setDropdownVisible(false)}>
                  <View style={[ styles.searchcard,
                      { top: modalPosition.y - -20, left: modalPosition.x - 12, width: "42%", height: 150, gap: 15, paddingHorizontal: 15 },
                    ]}
                  >
                    {statuses.map((status) => (
...
{/*--------------------------------- Floating Add Button -------------------------------------------------------------------------------------->*/}
          ...
          <Modal visible={addEmpModal} onRequestClose={() => setAddEmpModal(false)}>
            <View style={[styles.container, styles.paddinghorizontal, styles.flex]}>
              ...
              <ScrollView contentContainerStyle={[styles.paddingbottom, {flexGrow: 1}]}>
                <View style={[styles.inputcard, styles.gap, styles.width100]}>
...
{/*--------------------------------- Live Tracking Modal -------------------------------------------------------------------------------------->*/}
          ...
          <Modal visible={historyModalVisible} transparent onRequestClose={() => setHistoryModalVisible(false)} >
            <View style={[styles.employeecard, styles.centeralign, styles.width100]}>
              <View style={[styles.statusframe, styles.gap, styles.width100, {maxHeight:350}]}>
