import { addClient, fetchClientImage, getClients, updateClient } from "@/app/backend-api/api";
...
const Clients = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      const formattedClients: ClientType[] = data?.clients || [];

      // Show client data immediately
      setClients(formattedClients);

      // Fetch all client images in parallel
      await Promise.all(
        formattedClients.map(async (c) => {
          if (c._id) {
            try {
              const img = await fetchClientImage(c._id);
              if (img) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Build fresh payload every time
      const payload: any = {
        name,
      ...
      if (clientImage) {
        payload.imageUri = clientImage; // keep reference until axios call completes
      }

      let res: any = null;

      if (editingClientId) {
        res = await updateClient(editingClientId, payload);
      } else {
      ...
      resetForm(); // now reset modal & image
    } catch (error: any) {
      console.error("❌ Client Save Error:", error);
      Alert.alert(
        "❌ Error",
        error?.response?.data?.message || error.message || "Failed to save client"
      );
    } finally {
      setLoading(false);
    }
            ...
            <View style={[styles.employeecard, styles.centeralign]}>
              {selectedClient && (
                <View style={[styles.statusframe, styles.gap, styles.width100]}>
                  <Image
                    source={clientImages[selectedClient._id]
                  ...
                  <View style={[styles.rowitem, styles.centeralign]}>
                    <TouchableOpacity
                      onPress={() => { setSelectedClient(null); handleEditClient(selectedClient); }}
          ...
          <SafeAreaView style={styles.flex}>
            <View style={[styles.paddinghorizontal, styles.flex]}>
              ...
              <View style={[styles.mtop, styles.marginBottom1]}>
                <Btn
                  title={loading ? "Saving..." : editingClientId ? "Update Client" : "Add Client"}
