...
import { getAllPlans, upgradePlan } from "@/app/backend-api/api";
      ...
      <div className="flex flex-1 justify-center items-center w-full">
        <div className="flex flex-wrap justify-center gap-6 w-full">
          {Object.entries(plans).map(([planName, features]) => (
            <div key={planName} className="sm:w-[80%] md:w-[60%] lg:w-[23%] p-6 rounded-3xl bg-(--color-sidebar) shadow-md hover:shadow-[#8d8d8d] hover:scale-105 hover:rotate-2 transition-hover duration-300 flex flex-col">
              {/* Plan Name */}
              ...
              <div className="flex gap-2 justify-center ">
                {planName !== "trial" ? (
                  <>
                    <Button
                      title={buttonLoading === planName + "-monthly" ? "Upgrading..." : "Monthly"}
                      onClick={() => handleUpgrade(planName, "monthly")}
                      disabled={buttonLoading !== null}
                    />
                    <Button
                      title={buttonLoading === planName + "-yearly" ? "Upgrading..." : "Yearly"}
                      onClick={() => handleUpgrade(planName, "yearly")}
                      disabled={buttonLoading !== null} />
                  </>
                ) : (
                  <Button title={buttonLoading === planName + "-trial" ? "Activating..." : "Get Access"}
