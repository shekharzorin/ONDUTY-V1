import {
  adminAddEmployee, adminDeleteEmployee, adminGetEmployees, adminGetVisits, fetchEmployeeProfileMeta,
  fetchEmployeeProfilePhoto, fetchVisitImageBase64, getAdminDashboard, getAllLiveLocations,
  getEmployeeLiveLocation, getEmployeeLocationHistory
...
const EmployeeMap = memo(
  ({ employee, coordinates, groupedHistory, selectedDate }: EmployeeMapProps) => {
    ...
    // 🗓 Auto-center when switching between live/history
    useEffect(() => {
      if (!mapRef.current) return;
    ...
    }, [selectedDate, dailyCoords.length, coordinates.latitude, coordinates.longitude]);
    ...
    return (
      <View>
        ...
        <View style={styles.map}>
          ...
          >
            <Marker coordinate={selectedDate && dailyCoords.length > 0
              ? dailyCoords[dailyCoords.length - 1]
              : {
                latitude: coordinates.latitude,
                longitude: coordinates.longitude,
              }
            ...
            {dailyCoords.length > 1 && (
              <Polyline coordinates={dailyCoords} strokeColor="#007AFF" strokeWidth={4} />
            )}
          </MapView>
        ...
        <View style={styles.padding15}>
          <Text style={[styles.locationtxt, styles.color]}>{address}</Text>
        </View>
      </View>
    );
  }
...
function Track() {
  ...
  // Live locations map reducer
  const [liveLocationsMap, dispatchLocations] = React.useReducer(
    (
      state: Record<string, any>,
      action: {
        type: "update";
        payload: { email: string; latitude: number; longitude: number; timestamp?: Date };
      }
    ) => {
      switch (action.type) {
        case "update":
  ...
  const openModal = () => {
    statusButtonRef.current?.measure((_fx, _fy, _w, _h, px, py) => {
  ...
  const storeClockInTime = async (empEmail: string, time: string) => {
    try {
      const today = new Date().toLocaleDateString("en-GB");
      await AsyncStorage.setItem(
        `clockIn_${empEmail}`,
        JSON.stringify({ time, date: today })
      );
    } catch (e) {
  ...
  const getStoredClockInTime = async (empEmail: string) => {
    try {
      const data = await AsyncStorage.getItem(`clockIn_${empEmail}`);
      if (!data) return null;
      ...
      await AsyncStorage.removeItem(`clockIn_${empEmail}`);
      return null;
    } catch (e) {
  ...
  useEffect(() => {
    ...
    // 🔌 Connection logs (same as Next.js)
    const onConnect = () => {
      console.log("🟢 Socket connected (RN):", socket.id);
    };

    const onDisconnect = (reason: string) => {
      console.log("🔴 Socket disconnected (RN):", reason);
    };

    const onConnectError = (err: any) => {
      console.error(
        "❌ Socket connection error (RN):",
        err?.message || err
      );
    };
    ...
    const handleLocationUpdate = (data: any) => {
      console.log("📍 Live update (RN):", data);
    ...
    console.log("📡 Socket listeners attached (RN)");

    return () => {
      // 🧹 Cleanup listeners ONLY (do NOT disconnect)
      socket.off("connect", onConnect);
      socket.off("disconnect", onDisconnect);
      socket.off("connect_error", onConnectError);
      socket.off("locationUpdate", handleLocationUpdate);

      console.log("🔌 Socket listeners removed (RN)");
    };
  }, []);
  ...
  // When modal selected employee changes, fetch visits for them
  useEffect(() => {
    if (selectedEmployee?.email) {
      fetchEmployeeVisits(selectedEmployee.email);
    }
  }, [selectedEmployee?.email, fetchEmployeeVisits]);
  ...
  /* ------------------------------------------------------------------ */
  /* FETCH EMPLOYEES + STATUS + LIVE                                    */
  /* ------------------------------------------------------------------ */

  const fetchEmployeesWithStatus = useCallback(async (): Promise<void> => {
    setLoading(true);
    try {
      ...
      /* ======================================================
         ✅ FIX: USE ALL ACTIVITIES (NO DATE FILTER)
         Same as Next.js (THIS WAS THE BUG)
      ====================================================== */
      const allActivities: EmployeeActivity[] =
        dashData?.activities?.flatMap((g: any) => g.activities) || [];
      ...
      const enriched: Employee[] = await Promise.all(
        empRes.map(async (emp: any) => {
          ...
          const empActivities = allActivities
            .filter((a: any) => a.employeeEmail === emp.email)
            .sort(
              (a: any, b: any) =>
          ...
          /* ======================================================
             ✅ CORRECT STATUS LOGIC (SAME AS WEB)
          ====================================================== */
          const lastClockIn = empActivities.find(
            (a) => a.type === "clock-in"
          );
          const lastClockOut = empActivities.find(
            (a) => a.type === "clock-out"
          );
          ...
          const liveLoc = liveData.find(
            (l: any) => l.employeeEmail === emp.email
          );

          let clockInTime = formatTime(latest?.clockInTime);
          if (clockInTime) {
            await storeClockInTime(emp.email, clockInTime);
          } else {
          ...
          return {
            ...
            date: latest?.clockInTime
              ? new Date(latest.clockInTime).toLocaleDateString("en-GB")
              : new Date().toLocaleDateString("en-GB"),

            clockInTime,
            clockOutTime: formatTime(latest?.clockOutTime),
            clientCheckInTime: formatTime(latest?.clientCheckInTime),
            clientCheckOutTime: formatTime(latest?.clientCheckOutTime),
  ...
  /* ------------------------------------------------------------------ */
  /* EMPLOYEE LIVE + HISTORY                                            */
  /* ------------------------------------------------------------------ */

  const fetchEmployeeLiveAndHistory = useCallback(async (empEmail: string) => {
    try {
      ...
      const grouped = (history || []).reduce(
        (acc: Record<string, any[]>, loc: any) => {
          const date = new Date(loc.timestamp).toLocaleDateString("en-GB");
  ...
  const loadEmployeeMeta = useCallback(async (empEmail: string) => {
    const meta = await fetchEmployeeProfileMeta(empEmail);
          ...
          <View style={[styles.paddinghorizontal, styles.flex]}>
            ...
            <View style={styles.row}>
              <View style={styles.flex}>
                <Search searchText={searchText} setSearchText={setSearchText} placeholder="Search emp" />
              </View>
              ...
              <View style={[styles.searchcard, styles.h60, { width: "45%" }]}>
                <TouchableOpacity ref={statusButtonRef} onPress={openModal} style={{ paddingHorizontal: 5 }}>
                  <View style={styles.row}>
                    <Text style={[styles.text, styles.color, { marginTop: 3 }]} >
                      {selectedStatus}
                    </Text>
                ...
                <Modal visible={dropdownVisible} transparent onRequestClose={() => setDropdownVisible(false)} >
                  <TouchableOpacity activeOpacity={1} onPressOut={() => setDropdownVisible(false)} >
                    <View style={[styles.searchcard,
                    {
                      top: modalPosition.y + 55, left: modalPosition.x - 12, width: "42%",
                      height: 150, gap: 15, paddingHorizontal: 15,
                    },
                    ]} >
                      {statuses.map((status) => (
                        <TouchableOpacity key={status} style={styles.rowitem} onPress={() => handleSelect(status)} >
                          ...
                          {selectedStatus === status && (
                            <MaterialIcons name="check-circle" size={24} color="#646464" />
                          )}
                        </TouchableOpacity>
                      ))}
                    </View>
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 20 }} >
              {filteredEmployees.length === 0 ? (
                <View style={styles.centeralign}>
                  <Text style={[styles.locationtxt, styles.color, styles.margintop,]} >
                    No employees found
                  </Text>
                </View>
              ) : (
                  ...
                  <TouchableOpacity key={emp.email} onPress={() => openEmployeeModal(emp)}>
                    <View style={[styles.avatarframe, styles.marginBottom1]}>
                      <View style={[styles.rowitem, styles.centeralign]}>
                        <Image source={emp.profilePic ? { uri: emp.profilePic }
                        ...
                        <View style={[styles.gap5, styles.width48]}>
                          ...
                          {emp.clockInTime && (
                            <Text>Clock-in at {emp.clockInTime}</Text>
                          )}
                          {emp.clockOutTime && (
                            <Text>Clock-out at {emp.clockOutTime}</Text>
                          )}
                          {emp.clientCheckInTime && (
                            <Text>Last update at {emp.clientCheckInTime}</Text>
                          )}
                          {emp.clientCheckOutTime && (
                            <Text>Last update at {emp.clientCheckOutTime}</Text>
                          )}
                        </View>

                        <View style={[styles.rightalign, styles.centeralign]}>
                          <Text style={[styles.locationtxt,
                          { color: emp.status === "Active" ? "rgba(8, 143, 8, 1)" : "#e45d5dff", },]} >
                            {emp.status || "Inactive"}
                          </Text>
                        </View>
                      </View>
        ...
        <SafeAreaProvider style={styles.container}>
          <SafeAreaView style={styles.flex}>
            ...
            <View style={[styles.paddinghorizontal, styles.gap, { marginBottom: 15 },]} >
              <Btn title={addEmpLoading ? "Saving..." : "Add Employee"}
                onPress={handleAddEmployee} disabled={addEmpLoading}
              />
            </View>
          </SafeAreaView>
        </SafeAreaProvider>
                ...
                <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
                  <TouchableOpacity onPress={() => { setProfileModalVisible(true); loadEmployeeMeta(selectedEmployee.email); }}>
                    <View style={styles.avatarframe}>
                      <View style={[styles.rowitem, styles.centeralign]}>
                        <Image source={selectedEmployee.profilePic ? { uri: selectedEmployee.profilePic }
                          : require("../../assets/images/profile.webp")} style={[styles.avatarheight, styles.empavatar]}
                        />
                        <View style={[styles.gap5, styles.width48]}>
                          <Text style={styles.status}> {selectedEmployee.name} </Text>
                          {selectedEmployee.clockInTime && (
                            <Text> Clock-in at {selectedEmployee.clockInTime} </Text>
                          )}
                          {selectedEmployee.clockOutTime && (
                            <Text> Clock-out at {selectedEmployee.clockOutTime} </Text>
                          )}
                          {selectedEmployee.clientCheckInTime && (
                            <Text> Last update at{" "} {selectedEmployee.clientCheckInTime} </Text>
                          )}
                          {selectedEmployee.clientCheckOutTime && (
                            <Text> Last update at{" "} {selectedEmployee.clientCheckOutTime} </Text>
                          )}
                        </View>
                        <View style={[styles.rightalign, styles.centeralign]}>
                          <Text style={[styles.locationtxt,
                  ...
                  <View style={styles.margintop}>
                    {liveLocationsMap[selectedEmployee.email] ? (
                      <View style={styles.gap}>
                        ...
                        <EmployeeMap employee={selectedEmployee}
                          coordinates={{
                        ...
                        <FlatList
                          ...
                          renderItem={({ item }) => (
                            <View style={[styles.rowitem, styles.avatarframe, styles.centeralign, styles.marginBottom1,]}>
                              <Image source={visitImages[item._id] ? { uri: visitImages[item._id] }
                                : require("@/assets/images/profile.webp")
                              } style={[styles.avatarheight, styles.empavatar]} resizeMode="cover"
                              />
                              ...
                              <View style={styles.flex}>
                                ...
                                <View style={[styles.statusbg, { backgroundColor: item.status === "completed" ? "#ebfff1ff" : "#ffedeeff" }]}>
                                  <Text style={{
                                    color: item.status === "completed" ? "rgba(0, 147, 0, 1)"
                                      : "rgba(162, 0, 0, 1)", fontWeight: "500", bottom: 1,
                                  }}>
                                    {item.status}
                                  </Text>
                                </View>
                  ...
                  <View style={[styles.employeecard, styles.centeralign, styles.width100,]} >
                    <View style={[styles.statusframe, styles.gap, styles.width100]}>
                      <View style={styles.rowitem}>
                        <Image source={selectedEmployee.profilePic ? { uri: selectedEmployee.profilePic }
                          : require("../../assets/images/profile.webp")} style={styles.employeeprofile}
                        />
                      ...
                      <View style={styles.rowitem}>
                        ...
                        <Text style={[styles.locationtxt, styles.rightalign]}>{selectedEmployee?.email || "Not found"}</Text>
                      </View>

                      <View style={styles.rowitem}>
                        <Text style={styles.locationtxt}>Clock In :</Text>
                        <Text style={[styles.locationtxt, styles.rightalign]}>{selectedEmployee.clockInTime || "Not yet"}</Text>
                      </View>

                      <View style={styles.rowitem}>
                        <Text style={styles.locationtxt}>Clock Out :</Text>
                        <Text style={[styles.locationtxt, styles.rightalign]}>{selectedEmployee.clockOutTime || "Not yet"} </Text>
                      </View>

                      <View style={styles.rowitem}>
                        <Text style={styles.locationtxt}>Worked Hours :</Text>
                        <Text style={[styles.locationtxt, styles.rightalign]}>{selectedEmployee.workedHours || "Calculating ..."}</Text>
                      </View>

                      <View style={styles.rowitem}>
                        <Text style={styles.locationtxt}>Total Tasks :</Text>
                        <Text style={[styles.locationtxt, styles.rightalign]}>{totalVisitCount}</Text>
                      </View>

                      <View style={styles.rowitem}>
                        <Text style={styles.locationtxt}>Completed Tasks :</Text>
                        <Text style={[styles.locationtxt, styles.rightalign]}>{completedVisits}</Text>
                      </View>

                      <Btn title={deleting ? "Deleting..." : "Delete Employee"}
                        onPress={() => handleDeleteEmployee(selectedEmployee.email)} disabled={deleting}
                      />
                  ...
                  <View style={[styles.statusframe, styles.gap, styles.width100, { maxHeight: 350, minHeight: 350 },]} >
                    ...
                    <ScrollView>
                      <TouchableOpacity style={[styles.rowitem, styles.paddinghorizontal, styles.marginVertical,]}
                        ...
                        <Text style={[styles.locationtxt, { color: "rgba(8,143,8,1)" }]}>Go to Live Location</Text>
