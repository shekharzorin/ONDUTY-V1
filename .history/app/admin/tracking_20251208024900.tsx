...
function Track() {
  ...
  const EmployeeMap = ({
    ...
    // Compute daily coordinates for selected date
    const dailyCoords =
    ...
    // Reverse geocoding for employee location
    useEffect(() => {
      const getAddress = async () => {
        try {
          ...
          if (geocode && geocode.length > 0) {
            ...
            const fullAddress = `${streetLine}, ${loc.region || ""}, ${loc.country || ""} ${loc.postalCode ? "- " + loc.postalCode : ""
              }`;
    ...
    // Function to center map when user presses button (animated)
    ...
    // Auto-update map without animation (prevents jumping/flicker)
    useEffect(() => {
      if (mapRef.current) {
        const target =
          selectedDate && dailyCoords.length > 0
