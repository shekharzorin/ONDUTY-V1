...
const Client = () => {
  ...
  // Updated fetchClients with debug
const fetchClients = async () => {
  try {
    setLoading(true);

    const response = await adminGetClientsDebug(); // use debug wrapper

    if (!response || !response.clients) {
      console.log("⚠️ No clients found or response is null:", response);
      setClients([]);
      setClientImages({});
      return;
    }
    ...
    await Promise.all(
      formattedClients.map(async (c: any) => {
        if (!c._id) return;
        ...
        try {
          ...
          console.log(`⚠️ No image for client ${c._id}`, err);
        }
      })
    );
  } catch (error) {
