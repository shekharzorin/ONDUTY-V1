...
import AdminLocationTracker from "@/app/services/AdminTracker"; // ✅ ADD
...
function AdminLayout() {
  ...
  useEffect(() => {
    let mounted = true; // ✅ ADD (safety)

    const checkAuth = async () => {
      const valid = await validateToken();

      if (!valid) {
        // 🛑 stop tracking if token invalid
        AdminLocationTracker.stopTracking(); // ✅ ADD
      ...
      await AdminLocationTracker.startTracking(); // ✅ ADD
    ...
    return () => {
      mounted = false;
      // 🛑 cleanup when admin layout unmounts
      AdminLocationTracker.stopTracking(); // ✅ ADD
    };
  }, []);
