...
const EmployeeMap: React.FC<EmployeeMapProps> = ({
  ...
  // Reverse geocoding every 30 seconds
  useEffect(() => {
    let lastGeocodeTime = 0;

    const fetchAddress = async () => {
      const now = Date.now();
      if (now - lastGeocodeTime < 30000) return; // 30 seconds
      lastGeocodeTime = now;
    ...
    fetchAddress();
    const interval = setInterval(fetchAddress, 30000); // every 30 seconds
    return () => clearInterval(interval);
  }, [currentCoords]);

  // Center map on button press
