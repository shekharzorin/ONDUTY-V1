import { adminAddClient, adminDeleteClient, adminGetClients, adminUpdateClient, fetchClientImageBase64 } from "@/app/backend-api/api";
...
const Client = () => {
    ...
    try {
      ...
      if (data) {
        ...
        for (const c of data) {
          if (c._id) {
            try {
              const img = await fetchClientImageBase64(c._id);
              if (img) imgs[c._id] = img;
            } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      if (editingClientId) {
        await adminUpdateClient(editingClientId, {
          name,
  ...
  const handleCloseModal = () => {
    ...
    setEditingClientId(null);  // ✅ reset edit mode
    setName("");
    setNumber("");
  ...
  const handleEditClient = async (client: ClientType) => {
    ...
    try {
      const img = await fetchClientImageBase64(client._id);
      if (img) setClientImage(img);
      else setClientImage(null);
    } catch (err) {
      console.log("⚠️ No image for client:", err);
      setClientImage(null);
    }
  };
  ...
  return (
    <SafeAreaProvider style={styles.container}>
      <SafeAreaView style={styles.flex}>
              ...
              >
                <View style={[styles.rowitem, styles.flex]}>
                  <View style={styles.centeralign}>
                    <Image source={ clientImages[item._id]
                          ? { uri: clientImages[item._id] }
                          : require("@/assets/images/org.png")
                      } style={[styles.avatarheight, styles.empavatar]}
                    />
                  </View>
                  <View style={[styles.gap5, styles.width80, { justifyContent: "center" }]}>
                    <Text style={[styles.status]}>{item.name}</Text>
                    <View style={[styles.rowitem, { alignItems: "center" }]}>
                      <Ionicons name="location-sharp" size={24} color="#646464" />
                      <Text style={[styles.color, styles.width70]}>{item.address}</Text>
                    </View>
                  </View>
                </View>
              </TouchableOpacity>
        ...
        {/* 🔹 Client Detail Modal */}
        <Modal visible={!!selectedClient} transparent animationType="fade" onRequestClose={() => setSelectedClient(null)}>
          <View style={[styles.employeecard, styles.centeralign, { backgroundColor: 'rgba(213, 0, 0, 0.14)' }]}>
            {selectedClient && (
              <View style={[styles.statusframe, styles.gap, styles.width100]}>
                <Image source={ clientImages[selectedClient._id] ? { uri: clientImages[selectedClient._id] }
                      : require("@/assets/images/org.png")
                  } resizeMode="cover" 
                  style={styles.clientbanner}
                />

                <Text style={[styles.status]}>Client : {selectedClient.name}</Text>
                <Text style={[styles.plantxt, styles.color]}>Client Location : {selectedClient.address}</Text>
                <Text style={[styles.plantxt, styles.color]}>Client Number : {selectedClient.clientNumber}</Text>

                <View style={[styles.rowitem, styles.centeralign]}>
                  <TouchableOpacity onPress={() => {setSelectedClient(null); handleEditClient(selectedClient);}} style={styles.rightalign}>
                  ...
                  <TouchableOpacity onPress={() => handleDelete(selectedClient._id)}>
                    <View style={[ styles.bellicon, styles.centeralign,styles.iconframe]}>
                ...
                <TouchableOpacity onPress={() => setSelectedClient(null)} style={[styles.cross]}>
        ...
        <Modal visible={modalVisible} onRequestClose={() => handleCloseModal()}>
          <View style={[styles.paddinghorizontal, styles.container, styles.flex]}>
            
            <History title={editingClientId ? "Edit Client" : "Add Client"} onPress={() => handleCloseModal()} />
            ...
            <View style={[styles.mtop, styles.marginBottom1]}>
              <Btn title={ loading ? "Saving...": editingClientId? "Update Client": "Add Client"}
