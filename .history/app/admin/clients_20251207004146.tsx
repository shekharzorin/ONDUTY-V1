...
const Client = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      // ADMIN → full client list, EMPLOYEE → only assigned clients
      response = await adminGetClients();

      const formattedClients = Array.isArray(response) ? response : [];
      setClients(formattedClients);

      // Fetch all client images
      const imgs: Record<string, string> = {};

      for (const c of formattedClients) {
        try {
          if (c._id) {
          ...
          console.log("⚠️ No image for:", c._id);
        }
      }
