...
function Track() {
  ...
  const EmployeeMap = ({ employee, coordinates, groupedHistory, selectedDate }: {
    employee: Employee;
    ...
    const dailyCoords =
      selectedDate && groupedHistory[selectedDate]
        ? groupedHistory[selectedDate].map((loc) => ({ latitude: loc.latitude, longitude: loc.longitude }))
        : [];
    ...
    useEffect(() => {
      ...
      const getAddress = async () => {
        ...
        try {
          ...
          if (geocode.length > 0) {
            const loc = geocode[0];
            const streetParts = [loc.name, loc.street, loc.subregion, loc.city, loc.region].filter(Boolean);
    ...
    const centerToEmployee = () => {
      ...
      const target = selectedDate && dailyCoords.length > 0 ? dailyCoords[dailyCoords.length - 1] : currentCoords;
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={styles.map}>
          ...
          >
            <Marker
              coordinate={selectedDate && dailyCoords.length > 0 ? dailyCoords[dailyCoords.length - 1] : currentCoords}
              title={selectedDate ? `${employee.name} (${selectedDate})` : `${employee.name} (Live)`}
            />
            {dailyCoords.length > 1 && <Polyline coordinates={dailyCoords} strokeColor="#007AFF" strokeWidth={4} />}
          </MapView>
