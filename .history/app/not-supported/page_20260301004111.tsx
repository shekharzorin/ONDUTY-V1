    ...
    <div className="flex flex-col gap-5 items-center justify-center p-10 text-center h-screen font-semibold">
      ...
      {platform !== "other" && (
        <>
          {
            platform === "android"
              ? "Click the button below to download for Android / Tablet"
              : "Click the button below to download for iOS / iPad"
          }
          <a
            href={getDownloadLink()}
            className="bg-(--color-primary) text-white px-5 py-2 rounded-lg"
          >
            Download for{" "}
            {platform === "android" ? <>Hai</> : <>Bai</>}
          </a>
        </>
      )}
    </div>
