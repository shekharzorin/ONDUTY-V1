"use client";

import { motion } from "framer-motion";
import Crossicon from "../ui/Crossicon";
import { MdCall, MdLocationOn } from "react-icons/md";
import { LocateIcon, Mail, Phone } from "lucide-react";
import { TbMailFilled } from "react-icons/tb";
import { CiLocationOn } from "react-icons/ci";

interface ContactModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const ContactModal = ({ isOpen, onClose }: ContactModalProps) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-linear-to-r from-[#eccfff]/50 to-[#fff4da]/50 ">
      <motion.div
        initial={{ opacity: 0, scale: 0.5 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ type: "spring", stiffness: 100, damping: 20 }}
        className="relative w-[90%] max-w-md bg-transparent rounded-3xl shadow-2xl p-7 border border-white/50 bg-linear-to-r from-[#eccfff]/50 to-[#fff4da]/50 flex flex-col gap-10"
      >
        {/* Close */}
        <div className="absolute top-4 right-4">
          <Crossicon onClick={onClose} />
        </div>

        <div className="flex flex-col gap-3">
          <p className="text-2xl lg:text-3xl font-bold">Connect with OnDuty</p>
          <p className="md:text-lg text-(--color-gray)">
            Have questions about OnDuty? We're here to help.
          </p>
        </div>

        <div className="flex flex-col gap-5 px-3">
          <div className="flex gap-5 items-center">
            <MdCall
              size={45}
              className="p-2 rounded-full bg-[#fff0cd] text-(--color-gray) hover:scale-110 transition-all duration-300"
            />
            <div className="flex flex-col font-semibold md:text-lg">
              <p className="text-(--color-gray)">Number :</p>
              <p>+91 999 999 9999</p>
            </div>
          </div>

          <div className="flex gap-5 items-center">
            <TbMailFilled
              size={45}
              className="p-2.25 rounded-full bg-[#fff0cd] text-(--color-gray) hover:scale-110 transition-all duration-300"
            />
            <div className="flex flex-col font-semibold md:text-lg">
              <p className="text-(--color-gray)">Mail :</p>
              <p>onduty@gmail.com</p>
            </div>
          </div>

          <div className="flex gap-5 items-center">
            <MdLocationOn
              size={45}
              className="p-1.75 rounded-full bg-[#fff0cd] text-(--color-gray) hover:scale-110 transition-all duration-300"
            />
            <div className="flex flex-col font-semibold md:text-lg">
              <p className="text-(--color-gray)">Office :</p>
              <p>Siddipet IT Towert 2nd floor</p>
            </div>
          </div>
        </div>

        {/* Your form here */}
      </motion.div>
    </div>
  );
};

export default ContactModal;
