...
export default function Report({ route }: any) {
  ...
  const fetchReports = async () => {
    try {
      const data = await getReports(); // data: any[]
      if (data) {
        // Show report data immediately
        setReports(data);

        // Start fetching images in the background
        const imgs: Record<string, string> = {};
        data.forEach(async (r: any) => {  // <-- explicitly type r
          if (r._id) {
            try {
              const img = await fetchReportImageBase64(r._id);
              if (img) {
                imgs[r._id] = img;
                // Update state progressively as each image is loaded
