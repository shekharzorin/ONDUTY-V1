...
const Dashboard = () => {
  ...
  const handleClientToggle = async () => {
    try {
      // 🚨 HARD GUARD 1: Must be clocked-in
      if (!isRunning) {
        Alert.alert(
          "⛔ Action Not Allowed",
      ...
      // 🚨 HARD GUARD 2: Ensure active session exists TODAY
      const storedStart = await AsyncStorage.getItem("clockInTime");
      const storedDate = await AsyncStorage.getItem("clockDate");
      const today = new Date().toISOString().split("T")[0];
      ...
      if (!onJourney) {
        ...
        Alert.alert(
          "💼 Client Check-In",
          "You have successfully checked in to the client location.\n     \n— Have a productive meeting!"
        );
      } else {
