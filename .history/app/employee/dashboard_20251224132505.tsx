...
const Dashboard = () => {
  ...
  const handleClockIn = async () => {
    try {
      ...
      await AsyncStorage.setItem("clockInTime", now.getTime().toString());
      await AsyncStorage.setItem("clockDate", now.toISOString().split("T")[0]);

      const res = await sendDashboardAction({
        type: "clock-in",
        status: "Active",
        timestamp: now.toISOString(), // ✅ ONLY SOURCE OF TIME
      });
  ...
  const handleClockOut = async () => {
    try {
      ...
      const diff = Math.floor((now.getTime() - parseInt(storedStart, 10)) / 1000);
      const formattedDuration = formatTime(diff);

      await AsyncStorage.removeItem("clockInTime");
      await AsyncStorage.setItem("elapsedTime", diff.toString());
