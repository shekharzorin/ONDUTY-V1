...
export default function Report({ route }: any) {
 ...
 const fetchReports = async () => {
  try {
    ...
    setReports(data); // Set reports first so UI loads instantly

    // Fetch all images in parallel
    const imagePromises = data.map(async (report) => {
      if (!report._id) return null;
      try {
        const img = await fetchReportImageBase64(report._id);
        return { id: report._id, img };
      } catch {
        return null;
      }
    });
    ...
    // Create { reportId: base64 } map
    const imgs: Record<string, string> = {};
    results.forEach((res) => {
      if (res && res.img) imgs[res.id] = res.img;
    });

    setReportImages(imgs); // update state with fast-parallel images
