import { getClients, getProfile, getProfileImage, getVisits, sendDashboardAction } from "@/app/backend-api/api";
...
import { getReports } from "../backend-api/api";
...
// 🟢 Clock-In ----------------------------------------------------------------------------------------------------------------------------------->
  ...
  const handleClockIn = async () => {
    try {
      Alert.alert("🤝 Good Day!", "You have Clocked-In \n     \n— let’s make today productive!");
      ...
      const res = await sendDashboardAction({ type: "clock-in",status: "Active",});
      if (res.success) {
...
// 🔴 Clock-Out ---------------------------------------------------------------------------------------------------------------------------------->
  ...
  const handleClockOut = async () => {
    try {
      Alert.alert("👋 Good Job!", "You have Clocked-Out \n     \n— see you next time dear!");
      const storedStart = await AsyncStorage.getItem("clockInTime");
      ...
      const formattedDuration = `${hours.toString().padStart(2, "0")}h:${minutes
        .toString()
        .padStart(2, "0")}m:${secondsR.toString().padStart(2, "0")}s`;

      // ✅ Get current date & formatted time for display
      const { dateString, timeString } = getFormattedDateTime();

      setLoading(true);

      // ✅ Send to backend (no workedHoursDecimal)
...
{/* Notifications -----------------------------------------------------------------------------------------------------------------------------> */}
          ...
          <Modal visible={notificationModalVisible} animationType="slide" transparent={false} 
