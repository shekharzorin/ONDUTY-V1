import { fetchReportImageBase64, getReports, postReport, updateReport } from "@/app/backend-api/api";
...
export default function Report({ route }: any) {
  ...
  const fetchReports = async (pageNumber = 0) => {
    try {
      ...
      const res = await getReports(skip, limit); // <-- modify getReports to accept skip & limit
      const data = res || [];
      if (data.length < limit) setHasMore(false);

      // Append or reset reports
      setReports((prev) => (pageNumber === 0 ? data : [...prev, ...data]));

      // Fetch images in parallel without blocking
      data.forEach(async (r) => {
        if (r._id && !reportImages[r._id]) {
          try {
            const img = await fetchReportImageBase64(r._id);
            if (img) {
              setReportImages((prev) => ({ ...prev, [r._id]: img }));
            }
          } catch {
            console.log(`⚠️ No image for report ${r._id}`);
          }
        }
      });
    } catch (error) {
  ...
  const handleEditReport = async (report: any) => {
    ...
    try {
      ...
      if (img) setReportImageUri(img);
      else setReportImageUri(null);
    } catch (err) {
      console.log("⚠️ No image for report:", err);
      setReportImageUri(null);
    }
  };
