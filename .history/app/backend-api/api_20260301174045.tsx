...
export const API_BASE_URL = "http://10.235.140.49:5000";
...
/* -------------------------------------------------
   GET MAP CONFIG (ANDROID + IOS + WEB)
------------------------------------------------ */
export const getMapConfig = async () => {
  try {
    ...
    return {
      ...
      androidMapKey: res.data?.config?.androidMapKey || null,
      iosMapKey: res.data?.config?.iosMapKey || null,
      webMapKey: res.data?.config?.webMapKey || null,
    };
  } catch (err: any) {
    console.error(
      "❌ Fetch Map Config Error:",
      err.response?.data || err.message,
    );
