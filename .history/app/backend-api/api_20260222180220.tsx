...
export const API_BASE_URL = "http://10.237.107.49:5000";
...
/* -------------------------------------------------
 GET MAP CONFIG (EMPLOYEE / ADMIN)
 Fetch Google Map API key + map enabled status
------------------------------------------------ */
export const getMapConfig = async () => {
  try {
    ...
    return {
      ...
      googleMapKey: res.data?.config?.googleMapKey || null,
    };
  } catch (err: any) {
    console.error(
