...
const page = () => {
  ...
  const fetchMapConfig = async () => {
    try {
      const res = await fetch(`${API}/superadminmapapi/map-api`, {
        credentials: "include",
      });
      ...
      if (data.success) {
        setKeyName(data.data.key_name || "");
        setApiKey(data.data.googleMapKey || "");
        setMapEnabled(data.data.mapEnabled || false);
      }
    } catch (err) {
