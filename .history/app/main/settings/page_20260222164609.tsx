  ...
  return (
    <div className="flex flex-col h-screen w-full p-5 gap-5">
      ...
      <div className="flex flex-col w-full bg-(--color-sidebar) text-(--color-gray) rounded-2xl shadow-lg p-5 text-[18px] font-semibold gap-5">
        ...
        <div
          className="flex items-center justify-end w-fit bg-gray-300 text-(--color-gray) hover:bg-(--color-secondary) hover:scale-110 hover:text-(--color-primary) p-2 rounded-full cursor-pointer transition-all shadow-lg"
          onClick={() => {}}
        >
          <MdDelete size={26} className="flex items-end"/>
        </div>
      </div>
    </div>
  );
