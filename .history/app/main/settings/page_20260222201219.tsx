...
const page = () => {
  ...
  const handleSave = async () => {
    if (!searchText) return alert("❌ Please enter API key");

    setLoading(true);

    const res = await saveMapSettings(searchText);
  ...
  return (
    <div className="flex flex-col h-screen w-full p-5 gap-5">
      ...
      {/* ADD MAP API */}
      <div className="flex flex-col w-full bg-(--color-sidebar) text-(--color-gray) rounded-3xl shadow-lg p-5 text-[18px] font-semibold gap-5">
        ...
        <input
          ...
          className="w-full px-5 outline-none rounded-lg p-2 bg-gray-200"
        />
      ...
      <div className="flex relative flex-col w-full bg-(--color-sidebar) text-(--color-gray) rounded-3xl shadow-lg p-5 text-[18px] font-semibold gap-5">
