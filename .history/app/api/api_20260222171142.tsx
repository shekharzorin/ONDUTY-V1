...
/* ------------------------------------------------------------------
   12) SAVE OR UPDATE MAP SETTINGS (SUPER ADMIN)
------------------------------------------------------------------ */
export const saveMapSettings = async (googleMapKey : string, mapEnabled : boolean) => {
  try {
    const res = await api.post("/superadminmapsettings/map-api", {
      googleMapKey: googleMapKey.trim(),
      mapEnabled,
    });

    return res.data;
  } catch (err : any) {
    console.error("❌ Save map settings error:", err.response?.data || err.message);

    return {
      success: false,
      message: err.response?.data?.message || "Failed to save map settings",
    };
  }
};
...
export const getMapSettings = async () => {
  try {
    ...
    console.error("❌ Get map settings error:", err.response?.data || err.message);
...
/* ------------------------------------------------------------------
   14) DELETE MAP SETTINGS
------------------------------------------------------------------ */
export const deleteMapSettings = async () => {
  try {
    const res = await api.delete("/superadminmapsettings/map-api");
    return res.data;
  } catch (err : any) {
    console.error("❌ Delete map settings error:", err.response?.data || err.message);

    return {
      success: false,
      message: err.response?.data?.message || "Failed to delete map settings",
    };
  }
};
