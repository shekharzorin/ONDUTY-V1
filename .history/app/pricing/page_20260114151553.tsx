...
import { Check, Cross, Slice } from "lucide-react";
        ...
        <div className="flex items-center justify-center gap-5 md:gap-10 flex-wrap">
          {Object.entries(plans).map(([planName, features]) => {
            ...
            return (
              <div
                key={planName}
                className="flex flex-col backdrop-blur-md border border-white bg-white/30 shadow-lg rounded-3xl w-full md:w-fit gap-3 p-10 hover:shadow-[#8d8d8d] hover:scale-105 hover:rotate-1 transition-hover duration-300"
              >
                ...
                <div className="flex">
                  <ul className="mb-4">
                    {Object.entries(featureLabels).map(([key, label]) => (
                      ...
                      >
                        {features[key] ? <Check className="h-5 w-5 text-green-600 stroke-3" /> : <Cross className="h-5 w-5 text-red-600 stroke-3" />} {label}
                      </p>
                    ))}
                  </ul>
                </div>
