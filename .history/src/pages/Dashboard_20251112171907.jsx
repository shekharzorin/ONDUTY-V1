...
const CompaniesPage = () => {
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const response = await getAllAdmins(); // returns { success, count, trialCount, data }
        
        const formatted = response.data.map((admin) => ({
          _id: admin._id,
          name: admin.name,
          email: admin.email,
        ...
        setTrialCount(response.trialCount); // ✅ store trial count
      } catch (error) {
