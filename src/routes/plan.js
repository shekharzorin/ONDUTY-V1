...
router.post("/upgrade-plan", verifyToken, async (req, res) => {
  try {
    ...
    await sendEmail(
      ...
      `Hello ${user.name}, your ${plan} plan is active until ${expiryDate.toDateString()}.`
    );
...
/* -------------------- Add Employee -------------------- */
router.post( "/add-employee", verifyToken, isAdmin, checkFeature("addEmployees"), async (req, res) => {
    try {
...
router.get("/employees", verifyToken, isAdmin, async (req, res) => {
  try {
    const employees = await User.find({ adminEmail: req.user.email }).select("-password");
...
/* -------------------- Delete Employee -------------------- */
router.delete("/employee/:employeeEmail", verifyToken, isAdmin, async (req, res) => {
  try {
