...
import {
  getProfileImage, fetchEmployeeProfilePhoto, getAdminDashboard, fetchEmployeeProfileMeta, getNotifications,
  getAdminClients, deleteNotification, clearAllNotifications, approveOrDeclineClient, adminGetVisits
} from "@/app/backend-api/api";
import DashboardCard from "@/app/components/Dashboardcard";
import { FaHandshake, FaUsers, FaUsersSlash, FaBriefcase, FaTrash, FaBell } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const finalActivities = activities.map((act: any) => ({
        ...
        name: metaMap[act.employeeEmail]?.profile?.name || act.employeeName || "Unknown",
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const emails = [
        ...new Set(
          notes.map((n: any) => n.employeeEmail).filter(Boolean)
        ),
      ];
  ...
  return (
    <div className="sm:hidden lg:flex flex-col h-screen w-full bg-(--color-primary)">
      {/* ------------------ TOP HEADER ------------------ */}
      <div className="flex h-[100px] w-full p-4 justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="flex justify-between w-[75px] h-[75px] overflow-hidden rounded-2xl relative">
            <Image src={profileImage} alt="Profile" width={60} height={60} className="w-full h-full object-cover" />
        ...
        <div className="mr-5 relative bg-(--color-sidebar) p-3 rounded-full hover:cursor-pointer hover:shadow-lg" onClick={() => { fetchNotifications(); setOpenModal(true); }}>
          <FaBell className="text-(--color-primary)" size={30} />
          {notifications.length > 0 && (<span className="absolute -top-0.5 right-1 w-3.5 h-3.5 bg-red-600 rounded-full animate-pulse"></span>)}
        </div>
      </div>
      ...
      <div className="flex flex-col bg-(--color-bg) w-full flex-1 overflow-y-auto rounded-t-4xl p-2.5 gap-5">
        ...
        <div className="grid gap-5 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 px-2.5 pb-2.5 pt-1.5">
          {todayActivities.length === 0 && (
            <p className="font-medium text-(--color-gray) ml-3">No activity yet...</p>
          )}

          {todayActivities.map((act, index) => (
            <div key={index} className="bg-(--color-sidebar) rounded-3xl p-4 shadow-lg hover:scale-105 transition hover:bg-gray-100">
              <div className="flex justify-between w-[75px] h-[75px]">
                <Image src={act.profilePic || defaultProfile} alt="Employee profile" width={75} height={75} className="rounded-xl  object-cover" />
              </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <p className="mt-1 font-medium text-(--color-gray)">{formatType(act.type)}</p>
                  {act.type === "clock-in" && (
                    <p className="mt-1 font-medium text-(--color-gray)">{act.clockInTime}</p>
                  )}
                  {act.type === "clock-out" && (
                    <p className="mt-1 font-medium text-(--color-gray)">{act.clockOutTime}</p>
                  )}
                  {act.type === "client-check-in" && (
                    <p className="mt-1 font-medium text-(--color-gray)">{act.checkInTime}</p>
                  )}
                  {act.type === "client-check-out" && (
                    <p className="mt-1 font-medium text-(--color-gray)">{act.checkOutTime}</p>
                  )}
                </div>

                <div className="mt-2">
                  <Gobtn onClick={() => { setSelectedEmployee(act); setOpenEmpModal(true); }} />
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-(--color-sidebar) p-5 rounded-4xl w-[400px] shadow-xl animate-fade-in">
              <div className="flex justify-between w-[140px] h-[140px]">
                <Image src={selectedEmployee.profilePic || defaultProfile} alt="Employee profile" width={140} height={140} className="rounded-xl object-cover" />
              </div>

              <p className="mt-6 text-[18px] font-semibold ">{selectedEmployee.name}</p>
              <p className="mt-2 font-medium text-(--color-gray) capitalize">Type : {selectedEmployee.type}</p>
              {selectedEmployee.type === "clock-in" && (
                <p className="mt-2 font-medium text-(--color-gray)">Clock In At : {selectedEmployee.clockInTime}</p>
              )}
              {selectedEmployee.type === "clock-out" && (
                <p className="mt-2 font-medium text-(--color-gray)">Clock Out At : {selectedEmployee.clockOutTime}</p>
              )}
              {(selectedEmployee?.type === "client-check-in" || selectedEmployee?.type === "client-check-out") && selectedEmployee?.clientName && (
                <p className="mt-2 font-medium text-(--color-gray)"> Client Name : {selectedEmployee.clientName}</p>
              )}
              {selectedEmployee.type === "client-check-in" && (
                <p className="mt-2 font-medium text-(--color-gray)">Client Check In At : {selectedEmployee.checkInTime}</p>
              )}
              {selectedEmployee.type === "client-check-out" && (
                <p className="mt-2 font-medium text-(--color-gray)">Client Check Out At : {selectedEmployee.checkOutTime}</p>
              )}
        ...
        {openModal && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-end">
            <div className="w-[35%] h-full bg-(--color-sidebar) shadow-2xl animate-slide-left">
              <Header title={"Notifications"} onClick={() => setOpenModal(false)} />

              <div className="px-5">
                {notifications.length > 0 && (
                  <button className="text-red-500 mb-4 cursor-pointer font-medium" onClick={handleClearAll} >
                    Clear All
                  </button>
                )}
                ...
                <div className="flex flex-col gap-4 overflow-y-auto h-[calc(100vh-160px)] pr-1">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((note) => (
                      <div key={note._id} className="flex justify-between items-center p-4 bg-(--color-bg) rounded-3xl">
                        <div className="flex gap-4">
                          <div className="relative w-[75px] h-[75px]">
                            <Image src={note.profilePic || "/profile.webp"} alt="Profile" fill className="rounded-2xl object-cover" />
                          </div>

                          <div>
                            {note.employeeName && (<p className="text-[18px] font-semibold ">{note.employeeName}</p>)}
                            <p className="font-medium text-(--color-gray) capitalize">{note.type.replace(/-/g, " ")}</p>
                            <p className="font-medium text-(--color-gray)">{note.date} - {note.time}</p>
                          </div>
                        </div>

                        {note.type === "client-pending" ? (
                          <div>
                            <Button title="View" onClick={() => { setSelectedClient(note); setOpenNotificationModal(true); }} />
                          </div>
                        ) : (
                          <Deletebtn onClick={() => handleDeleteNotification(note._id)} />
                        )}
                      </div>
                    ))
                  )}
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-white w-[420px] rounded-4xl p-6 shadow-2xl animate-fade-in relative">
              <div className="relative w-full h-[200px] rounded-2xl overflow-hidden">
                <Image src={selectedClient?.clientData?.image?.data
                  ? `data:${selectedClient.clientData.image.contentType};base64,${selectedClient.clientData.image.data}` : "/org.png"
                } alt="Client" fill className="object-cover"
                />
              </div>

              <p className="mt-6 text-[18px] font-semibold mb-2">Client Name : {selectedClient?.clientData?.name || "Unknown Client"}</p>
              <p className="mt-2 font-medium text-(--color-gray)">Address : {selectedClient?.clientData?.address || "N/A"}</p>
              <p className="mt-2 font-medium text-(--color-gray)">Number : {selectedClient?.clientData?.clientNumber || "N/A"}</p>

              <div className="flex gap-4 mt-6">
                <Button title="Approve" onClick={() => handleApproveDecline("approve")} />
                <Button title="Decline" onClick={() => handleApproveDecline("decline")} />
              </div>
